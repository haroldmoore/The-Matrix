/*
 * 
 * code changes noted for using a common anode or common cathode 8x8 led matrix
 * 12/13/23
 * 
 * 
 * 
    ...
    for (byte r = 0; r < DISPLAY_SIZE; r++)
    {
      ...
      for (byte c = 0; c < DISPLAY_SIZE; c++)
      {
        ...
        if (shouldTurnLedOn)
        {
          ...
          // different setups for common cathode or common anode 8x8 led matrix displays 
          digitalWrite(displayRowPins[r], LOW);             //low-common cathode, high-common adode
          digitalWrite(displayColumnPins[c], HIGH);         //high-common cathode, low-common anode 
        }
      }
      delay(frameWaitTime);
      //
...
void ScrollingText8x8Display::clearDisplay()
{
  for (byte i = 0; i < DISPLAY_SIZE; i++)
  {
    ...
    // different setups for common cathode or common anode 8x8 led matrix displays 
    digitalWrite(displayRowPins[i], HIGH);        //high-common cathode, low-common anode
    digitalWrite(displayColumnPins[i], LOW);      //low-common cathode, high-common adode
  }
}
